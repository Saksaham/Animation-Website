...
function Footer() {
  return (
    <div className="flex w-full h-screen px-14">
      <div className='w-1/2 h-full flex flex-col justify-between uppercase font-["Founders_Grotesk_X-Condensed"]'>
        <div className="heading">
          <h1 className="text-[10vw] uppercase leading-none -mb-10 font-semibold"> eye-</h1>
          <h1 className="text-[10vw] uppercase leading-none -mb-10 font-semibold">opening</h1>
        </div>
        <div className="mb-24">
      ...
      <div className="w-1/2 h-full gap-5 relative ">
        <h1 className="text-[9vw] leading-none -mb-10 font-semibold font-['Founders_Grotesk_X-Condensed'] uppercase">presentations</h1>
        
            <div className="absolute">
            {["S:" , "Instagram" , "Behance" , "Facebook" , "Linkedin" , "L:" , "202-1965 W 4ht Ave" , "Vancouver,Canada" , "30 Chukarina St" , "Lviv,Ukraine" , "E:" , "hello@ochi.design"].map((item,index)=><a className={`${index==0 && "py-10"} ${index==5 && "py-10"} ${index==10 && "py-10"}  font-extralight ${index == 0 ? "": index==5 ? "" : index==10 ? "" : "underline underline-offset-1"} block`} key={index}>{item}</a>)}
            <h6 className="mt-20 text-md font-extralight text-[#B2B2B2]">&copy; ochi design 2024.Legal Terms</h6>
           </div>
            <div className="absolute top-[37vh] right-1">
              {["M:" , "Home" , "Services" , "Our Work" , "About us" , "Insights" , "Contact us"].map((item,index)=><a key={item} className={`${index==0 && "py-10"} font-extralight ${index == 0 ? "" : "underline underline-offset-1"} block`}>{item}</a>)}
              <h6 className="mt-[20vh] text-md font-extralight text-[#B2B2B2]">Website by Obys</h6>
            </div>
